...
function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => window.localStorage.getItem('lime-theme') === 'dark')
  ...
  useEffect(() => {
    window.localStorage.setItem('lime-theme', darkMode ? 'dark' : 'light')
  }, [darkMode])
  ...
  return <AuroraBackground className={`app-shell ${darkMode ? 'theme-dark' : ''}`}>
    <header className="topbar"><div className="brand"><span className="brand-mark">LI</span><div><p>Lime</p><span>Gestión de laboratorio</span></div></div><div className="header-right"><div className={`connection-status ${apiOnline ? 'is-online' : ''}`}><span />{apiOnline ? 'API conectada' : 'API sin conexión'}</div><button className="theme-toggle" type="button" onClick={() => setDarkMode((current) => !current)} aria-label={darkMode ? 'Activar modo claro' : 'Activar modo oscuro'} aria-pressed={darkMode}><span className="theme-toggle-icon" aria-hidden="true">{darkMode ? '☀' : '◐'}</span><span>{darkMode ? 'Modo claro' : 'Modo oscuro'}</span></button></div></header>
